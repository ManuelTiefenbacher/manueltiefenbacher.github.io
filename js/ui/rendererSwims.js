...
class SwimRenderer {
    ...
    calculateDaysSinceRest() {
        const list = window.dataProcessor?.swims;
    ...
    renderAverageDistanceChart(activities, avgWeekly) {
        ...
        const canvas = document.getElementById("chartSwim");
        if (!canvas) return;
                    ...
                    tooltip: {
                        callbacks: {
                            label: (context) => {
                                ...
                                if (label === "Avg Pace (min/100m)") {
                                    const minutes = Math.floor(value);
                                    const seconds = Math.round(
                                        (value - minutes) * 60
                                    );
                                    return `${label}: ${minutes}:${seconds.toString().padStart(2, "0")}/100m`;
                                }
    ...
    renderIntensityChart(activities) {
        const canvas = document.getElementById("intensityChartSwim");
        if (!canvas) return;
        ...
        const inRange = window.dataProcessor.getSwimsInRange(daysToShow);
        const distribution =
