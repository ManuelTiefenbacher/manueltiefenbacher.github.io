...
class PeriodComparison {
  ...
  init() {
    ...
    if (generateBtn) {
      generateBtn.addEventListener('click', () => this.generateComparison());
    }
    ...
    if (periodLengthSelect) {
      periodLengthSelect.addEventListener('change', () => this.updatePeriodOptions());
    }
  }
  ...
  openComparisonModal() {
    ...
    if (modal) {
      ...
      setTimeout(() => {
        ...
        const savedPeriod1 = sessionStorage.getItem('comparison_period1');
        const savedPeriod2 = sessionStorage.getItem('comparison_period2');
    ...
    switch(length) {
      ...
      case '14':
        ...
        for (let i = 0; i < 12; i++) {
          ...
          options.push({
            value: `14-${periodsAgo}`,
            label: periodsAgo === 0 ? 'Last 14 Days' : 
                   periodsAgo === 1 ? '14 Days Ago' : 
      ...
      case '90':
        ...
        for (let i = 0; i < 8; i++) {
          ...
          options.push({
            ...
            label: quartersAgo === 0 ? 'This Quarter' : 
                   quartersAgo === 1 ? 'Last Quarter' : 
                   `Q${quarter} ${year}`
          });
        }
        break;
      ...
      case '180':
        ...
        for (let i = 0; i < 4; i++) {
          ...
          options.push({
            value: `180-${halfsAgo}`,
            label: halfsAgo === 0 ? 'Last 6 Months' : 
                   `H${half} ${year}`
          });
        }
        break;
      ...
      case '365':
        ...
        for (let i = 0; i < 3; i++) {
          ...
          options.push({
            value: `365-${yearsAgo}`,
            label: yearsAgo === 0 ? 'Last 12 Months' : 
                   `${year}`
          });
        }
        break;
  ...
  generateComparison() {
    ...
    if (period1 === period2) {
      window.feedbackManager.showError('Please select different periods to compare');
      return;
    }
  ...
  renderComparison(periodLength, period1Value, period2Value, sportType) {
    ...
    const period1Label = document.querySelector(`#period1Select option[value="${period1Value}"]`).textContent;
  ...
  renderZoneComparisonChart(data1, data2, label1, label2) {
    ...
    const data1Percentages = zones.map(z => data1.hrDistribution.percentages[z] || 0);
    const data2Percentages = zones.map(z => data2.hrDistribution.percentages[z] || 0);
