...
class RunClassifier {
  ...
  classifyFromDetailedHR(hrAnalysis) {
    ...
    const timeZ3toZ5 = hrAnalysis.percentZ3 + hrAnalysis.percentZ4 + hrAnalysis.percentZ5;
  ...
  getTrainingLoadStats(runs) {
    ...
    classifications.forEach(({ run, classification }) => {
      const category = classification.category.split(' (')[0].toLowerCase();
...
// Export singleton
window.runClassifier = new RunClassifier(window.dataProcessor, window.hrAnalyzer);