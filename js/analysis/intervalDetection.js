...
class IntervalDetector {
    constructor() {
        ...
        this.PACE_VARIATION_THRESHOLD = 0.1; // 10% variation - more sensitive for time-based intervals
    ...
    detectInterval(run) {
        ...
        const avgPace =
            coreSection.corePaces.reduce((sum, p) => sum + p, 0) /
            coreSection.corePaces.length;
        const variance =
            coreSection.corePaces.reduce(
                (sum, p) => sum + Math.pow(p - avgPace, 2),
                0
            ) / coreSection.corePaces.length;
        ...
        const segments = this.detectPaceSegments(
            coreSection.corePaces,
            avgPace,
            coreSection.coreTimes
        );
        ...
        const isInterval =
            (coefficientOfVariation > 0.12 && intervalCount >= 2) || // Lower CV threshold
    ...
    findCooldownStart(paces, movingAvg) {
        ...
        const middleAvg =
            paces.slice(middleStart, middleEnd).reduce((sum, p) => sum + p, 0) /
    ...
    detectPaceSegments(paces, avgPace, times) {
        ...
        speeds.forEach((speed, i) => {
            ...
            if (currentSegment === null) {
                ...
                (isFast && currentSegment === "slow") ||
                (!isFast && currentSegment === "fast")
            ) {
                ...
                if (duration >= 30 && segmentPaces.length >= 3) {
                    const avgSegmentPace =
                        segmentPaces.reduce((sum, p) => sum + p, 0) /
                        segmentPaces.length;
                    const avgSegmentSpeed =
                        segmentSpeeds.reduce((sum, s) => sum + s, 0) /
        ...
        if (fastSegments.length > 0 && slowSegments.length > 0) {
            const avgFastSpeed =
                fastSegments.reduce((sum, s) => sum + s.avgSpeed, 0) /
                fastSegments.length;
            const avgSlowSpeed =
                slowSegments.reduce((sum, s) => sum + s.avgSpeed, 0) /
                slowSegments.length;

            // Require at least 6% speed difference between fast and slow
            const speedDifference =
                (avgFastSpeed - avgSlowSpeed) / avgSlowSpeed;
        ...
        const avgFastPace =
            fastSegments.length > 0
                ? fastSegments.reduce((sum, s) => sum + s.avgPace, 0) /
                  fastSegments.length
                : avgPace;

        const avgSlowPace =
            slowSegments.length > 0
                ? slowSegments.reduce((sum, s) => sum + s.avgPace, 0) /
                  slowSegments.length
                : avgPace;
    ...
    calculateAveragePace(run) {
        if (run.paceStream && run.paceStream.pace) {
            const validPaces = run.paceStream.pace.filter(
                (p) => p > 0 && p < 20
            );
            if (validPaces.length > 0) {
                return (
                    validPaces.reduce((sum, p) => sum + p, 0) /
                    validPaces.length
                );
            }
        }
        ...
        if (run.distance > 0 && run.duration > 0) {
            return run.duration / 60 / run.distance; // Convert seconds to minutes, then min/km
        }
    ...
    calculateAverageSpeed(activity) {
        ...
        const distanceKm = activity.distance; // already in km
    ...
    calculateAverageSwimPace(activity) {
        ...
        const distanceMeters = activity.distance * 1000; // convert km to meters
        const durationMinutes = activity.duration / 60; // convert seconds to minutes

        if (distanceMeters === 0) return null;

        return (durationMinutes / distanceMeters) * 100; // min per 100m
    }

    formatSwimPace(pace) {
        // Format pace as min:sec/100m
        if (!pace) return "—";
