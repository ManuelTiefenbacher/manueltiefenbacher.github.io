...
class StorageManager {
        ...
        this._ready = new Promise((resolve, reject) => {
            ...
            req.onsuccess = () => {
                ...
                this.db.onversionchange = () => {
                    console.warn(
                        "IndexedDB version change detected. Closing DB."
                    );
                    try {
                        this.db.close();
                    } catch {}
    ...
    async saveRuns(runs) {
        try {
            ...
            const tx = this.db.transaction(
                [this.STORES.RUNS, this.STORES.META],
                "readwrite"
            );
    ...
    async loadRuns() {
        try {
            ...
            console.log(
                `✓ Loaded ${runs.length} runs from IndexedDB (updated: ${lastUpdated || "n/a"})`
            );
            return runs;
        } catch (err) {
    ...
    async loadSwims() {
        try {
            ...
            console.log(
                `✓ ${(swims || []).length} swims loaded from IndexedDB`
            );
            return swims || [];
        } catch (err) {
