...
class DataProcessor {
    ...
    addRuns(newRuns, source = "unknown") {
        ...
        this.runs = Array.from(uniqueMap.values()).sort(
            (a, b) => b.date - a.date
        );
    ...
    addSwims(newSwims, source = "unknown") {
        ...
        this.swims = Array.from(uniqueMap.values()).sort(
            (a, b) => b.date - a.date
        );
    ...
    hasMoreData(runA, runB) {
        ...
        if (aHasStream && bHasStream) {
            return (
                runA.hrStream.heartrate.length > runB.hrStream.heartrate.length
            );
        }
    ...
    calculateMaxHR() {
        ...
        if (maxHR > 0) {
            this.hrMax = maxHR;
            console.log(
                `❤️ Max HR: ${maxHR} bpm from ${maxActivity.date.toLocaleDateString()}`
            );
        }
    ...
    setZones(zones) {
        ...
        const valid =
            values.every((v) => typeof v === "number" && v > 0 && v < 1) &&
            values[0] < values[1] &&
            values[1] < values[2] &&
            values[2] < values[3];
    ...
    _computeSummary(list, getInRangeFn) {
        ...
        const distance28 = last28.reduce(
            (sum, a) => sum + (a.distance || 0),
            0
        );
        const distance180 = last180.reduce(
            (sum, a) => sum + (a.distance || 0),
            0
        );
        ...
        return {
            ...
            hrData: {
                ...
                withStreamHR: list.filter(
                    (a) => a.hrStream?.heartrate?.length > 0
                ).length,
            },
            paceData: {
                withPace: list.filter((a) => a.paceStream?.pace?.length > 0)
                    .length,
            },
        };
    }
    ...
    getSummaryRuns() {
        ...
        return {
            ...
            last7Days: {
                runs: summary.last7Days.count,
                distance: summary.last7Days.distance,
            },
            last28Days: {
                runs: summary.last28Days.count,
                distance: summary.last28Days.distance,
            },
            last6Months: {
                runs: summary.last6Months.count,
                distance: summary.last6Months.distance,
                avgWeekly: summary.last6Months.avgWeekly,
            },
            hrData: {
                maxHR: this.hrMax,
                runsWithBasicHR: summary.hrData.withBasicHR,
                runsWithStreamHR: summary.hrData.withStreamHR,
            },
    ...
    getSummaryRides() {
        ...
        return {
            ...
            last7Days: {
                rides: summary.last7Days.count,
                distance: summary.last7Days.distance,
            },
            last28Days: {
                rides: summary.last28Days.count,
                distance: summary.last28Days.distance,
            },
            last6Months: {
                rides: summary.last6Months.count,
                distance: summary.last6Months.distance,
                avgWeekly: summary.last6Months.avgWeekly,
            },
            hrData: {
                ridesWithBasicHR: summary.hrData.withBasicHR,
                ridesWithStreamHR: summary.hrData.withStreamHR,
            },
    ...
    getSummarySwims() {
        ...
        return {
            ...
            last7Days: {
                swims: summary.last7Days.count,
                distance: summary.last7Days.distance,
            },
            last28Days: {
                swims: summary.last28Days.count,
                distance: summary.last28Days.distance,
            },
            last6Months: {
                swims: summary.last6Months.count,
                distance: summary.last6Months.distance,
                avgWeekly: summary.last6Months.avgWeekly,
            },
            hrData: {
                swimsWithBasicHR: summary.hrData.withBasicHR,
                swimsWithStreamHR: summary.hrData.withStreamHR,
            },
    ...
    getSummaryAll() {
        ...
        const aggDistance7 =
            (runs.last7Days.distance || 0) +
            (rides.last7Days.distance || 0) +
            (swims.last7Days.distance || 0);
        const aggDistance28 =
            (runs.last28Days.distance || 0) +
            (rides.last28Days.distance || 0) +
            (swims.last28Days.distance || 0);
        const aggDistance180 =
            (runs.last6Months.distance || 0) +
            (rides.last6Months.distance || 0) +
            (swims.last6Months.distance || 0);
        ...
        return {
            ...
            last6Months: {
                ...
                avgWeekly:
                    (runs.last6Months.avgWeekly || 0) +
                    (rides.last6Months.avgWeekly || 0) +
                    (swims.last6Months.avgWeekly || 0),
